...
export class SubscriptionService {
  ...
  private async chargeSubscription(obj, param) {
   ...
   async payment(merchantId: string, stripeId: string, source: string, param, unsuccessFunction?) {
     const {plan, subscription: {quantityOfPos, endOfSubscription:  subscriptionEnds}} = await this.getPlanByMerchantId(merchantId);

     return this.stripeService.makeTransaction({ customer: stripeId, source, amount: quantityOfPos * plan.price * 100, currency: 'usd' })
       .then((charge) => {
         ...
         if (moment().isBefore(subscriptionEnds) && plan.period === 'yearly') {
           endOfSubscription = moment(subscriptionEnds).add(1, 'year').toISOString();
         } else if (moment().isBefore(subscriptionEnds)) {
           endOfSubscription = moment(subscriptionEnds).add(1, 'month').toISOString();
