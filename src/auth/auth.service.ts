import verifyAppleToken from '../../util/decodeAppleToken';
...
export class AuthService {
  ...
  async getAppleCreeds(token): Promise<{ email: string, id: string }> {
    try {
      const jwtClaims = await verifyAppleToken(token);
      return { email: _.get(jwtClaims, 'email'), id: _.get(jwtClaims, 'email') };
    } catch (e) {
      if (e.status === 400) {
        throw new BadRequestException('Apple creds is not valid');
      } else {
        throw new BadRequestException('Try it later');
      }
    }
  ...
  async validateAppleUser(email: string, id: string): Promise<DbUserDto> {
    const user = await this.usersService.getByEmail(email);
